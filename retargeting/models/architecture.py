...
class GAN_model(BaseModel):
    ...
    def forward_kai(self):
        ...
        for src in range(self.n_topology):
            for dst in range(self.n_topology):
                ...
                for param in self.models[dst].auto_encoder.enc.parameters():
                    param.requires_grad = False

                # temp_latent = self.latents[src].clone().detach()
                temp_latent = torch.nn.Parameter(
                    self.latents[src].clone().detach(), requires_grad=True
                )
                optimizer = torch.optim.Adam([temp_latent], lr=0.01)
                criterion = nn.MSELoss()
                target = self.res_pos[src][:, :, 22, :].clone().detach()
        ...
        for i in range(self.n_topology):
            ...
            self.loss_Ds.append(
                self.backward_D_basic(
                    self.models[i].discriminator, self.pos_ref[i].detach(), fake
                )
            )
    ...
    def backward_G(self):
        ...
        for i in range(self.n_topology):
            ...
            res_pos_global = self.models[i].fk.from_local_to_world(
                self.res_pos[i]
            ) / height.reshape(height.shape + (1,))
            ...
            rec_loss = (
                rec_loss1
                + (
                    rec_loss2 * self.args.lambda_global_pose
                    + rec_loss3 * self.args.lambda_position
                )
                * 100
            )
        ...
        for src in range(self.n_topology):
            for dst in range(self.n_topology):
                cycle_loss = self.criterion_cycle(
                    self.latents[src], self.fake_latent[p]
                )
                self.loss_recoder.add_scalar(
                    "cycle_loss_{}_{}".format(src, dst), cycle_loss
                )
                ...
                if src != dst:
                    ...
                    self.loss_recoder.add_scalar(
                        "G_loss_{}_{}".format(src, dst), loss_G
                    )
                    self.loss_G += loss_G
        ...
        self.loss_G_total = (
            self.rec_loss * self.args.lambda_rec
    ...
    def verbose(self):
        res = {
            "rec_loss_0": self.rec_losses[0].item(),
